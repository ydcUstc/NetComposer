...
from midi.util import *
from midi.reg import *
class FileReader(object):
    ...
    def parse_file_header(self, midifile):
        ...
        if magic != bytes('MThd','ASCII'):##'MThd':
    ...
    def parse_track_header(self, midifile):
        ...
        magic = midifile.read(4)
        if magic != bytes('MTrk','ASCII'):##'MTrk':
    ...
    def parse_midi_event(self, trackdata):
        ...
        stsmsg = next(trackdata)##ord(trackdata.next())
        # is the event a MetaEvent?
        if MetaEvent.is_event(stsmsg):
            cmd = next(trackdata)##ord(trackdata.next())
            ##if cmd != 0x51:
                ##cmd=cmd
            ...
            data = [next(trackdata) for x in range(datalen)]##[ord(trackdata.next()) for x in range(datalen)]
            text=[chr(data[i]) for i in range(datalen)]
            text="".join(text)
        ...
        # is this event a Sysex Event?
            ...
            data = []
            while True:
                datum = next(trackdata)##ord(trackdata.next())
                if datum == 0xF7:
                    break
                data.append(datum)
        ...
        # not a Meta MIDI event or a Sysex event, must be a general message
            ...
            if key not in EventRegistry.Events:
                ...
                data.append(stsmsg)
                data += [next(trackdata) for x in range(cls.length - 1)]##[ord(trackdata.next()) for x in range(cls.length - 1)]
                ...
                cls = EventRegistry.Events[key]()
                channel = self.RunningStatus & 0x0F
                data = [next(trackdata) for x in range(cls.length)]##[ord(trackdata.next()) for x in range(cls.length)]
                ##return cls(tick=tick, channel=channel, data=data)
                cls.tick = tick
                cls.data = data
                cls.channel = channel
...
def write_midifile(midifile, pattern):
    #if type(midifile) in (str, unicode):#unicode is not included in str in python 2
    if type(midifile) is (str):#unicode is included in str in python 3
